...
// If quorum is enabled, it waits for quorum before sending.
// Returns ctx.Err() in each result if context is cancelled before quorum is reached.
func (a *Alan) Send(ctx context.Context, data []byte) []SendResult {
	// Wait for quorum if enabled
	if err := a.waitForQuorum(ctx); err != nil {
		// Return error for all current peers
		peers := a.peers.list()
		if len(peers) == 0 {
...
// It waits for quorum before sending if quorum is enabled.
...
func (a *Alan) QuorumSize() int {
	if a.config.Quorum == 0 {
		return 0
	}
	return (a.config.Quorum / 2) + 1
}

// HasQuorum returns true if the current number of peers meets the quorum requirement.
// Always returns true if quorum is disabled (Quorum == 0).
...
func (a *Alan) handleLockGrant(requestID []byte, key string, sourceAddr *net.UDPAddr) {
	reqKey := hex.EncodeToString(requestID)
